...
namespace GLS
{
	...
	public:
		void data(unsigned int vs, float*v, unsigned int is, unsigned int*i, unsigned int t);
		void data(MODEL& model, unsigned int t);
		VideoModel();
		VideoModel(MODEL& model, unsigned int draw_type);
		VideoModel(MODEL& model, unsigned int draw_type, unsigned int each_size_size, unsigned int* each_size, unsigned int sumofthem);
		~VideoModel();
