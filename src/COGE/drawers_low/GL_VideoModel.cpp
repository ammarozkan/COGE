...
namespace GLS
{
	...
	void VideoModel::object_init(unsigned int v_size, float*ver, unsigned int i_size, unsigned int*ind, unsigned int type)
	{
		vao = new VAO(); vbo = new VBO(); ebo = new EBO();
		data(v_size,ver,i_size,ind,type);
	}

	void VideoModel::data(unsigned int v_size, float* ver, unsigned int i_size, unsigned int*ind, unsigned int type)
	{
		vao->bind();
	...
	void VideoModel::data(MODEL& model, unsigned int t)
	{
		data(model.vertice_size,model.vertices,model.indice_size,model.indices,t);
	}
	...
	VideoModel::VideoModel(MODEL& model, unsigned int draw_type) { init_buffers(model,draw_type); }

	VideoModel::VideoModel(MODEL& model, unsigned int draw_type, unsigned int each_size_size, unsigned int* each_size, unsigned int sumofthem=0) 
	{ init_buffers(model,draw_type,each_size_size,each_size,sumofthem); }

	void VideoModel::init_buffers(MODEL& model, unsigned int draw_type)
	{
	...
	void VideoModel::init_buffers(MODEL& model, unsigned int draw_type, unsigned int each_size_size, unsigned int* each_size, unsigned int sumofthem=0)
	{
		object_init(model.vertice_size,model.vertices,model.indice_size,model.indices,draw_type);
